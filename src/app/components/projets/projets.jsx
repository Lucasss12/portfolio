...
export default function Projets() {
  ...
  const handleMouseMove = (e) => {
    ...
    setRotationAngle({ x: angleX, y: angleY });

       // Calcul de la distance de la souris par rapport au centre de l'image
       const distance = Math.sqrt(mouseX ** 2 + mouseY ** 2);

       // Si la distance est supérieure à une certaine valeur, indiquez que la souris est suffisamment éloignée
       if (distance > 150) {
         setIsMouseAway(true);
       } else {
     ...
     const resetRotation = () => {
       if (isMouseAway) {
         // Réinitialisez l'angle de rotation lorsque la souris est suffisamment éloignée
         setRotationAngle({ x: 0, y: 0 });
       }
     }; 
